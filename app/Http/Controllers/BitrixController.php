...
{
    public function search($user_name,$address){
        return $this->_bitrix_curl(array(
            //'action'=>'order',
            'action'=>'search',
            //'order_id'=>'1002839',
            'user_name'=>$user_name,
    ...
    public function orders($date_begin,$date_end=''){
        $arParams=array(
            'action'=>'list',
            'date_begin'=>$date_begin,
        );
        if(!empty($date_end))$arParams['date_end']=$date_end;
        return $this->_bitrix_curl($arParams);
    }
    ...
    public function getNewOrders(){
        ...
        $bitrix=$this->_bitrix_curl($arParams);
        $arOrders=array();
    ...
    public function statusSet($number,$status){
        return $this->_bitrix_curl(array(
            'action'=>'status_set',
            'order_id'=>$number,
            'status'=>$status,
    ...
    public function statusGet($number){
        return $response = $this->_bitrix_curl(array(
            'action'=>'status_get',
            'order_id'=>$number,
        ));
    }
    public function statusList(){
        return  $this->_bitrix_curl(array('action'=>'status_list_get'));
    }

    private function _bitrix_curl($arParams){
        $json_send=json_encode($arParams);
        $ch=curl_init("http://www.santehsmart.ru/testzone/$json_send/elza.json");
